...
|---|---|---|
| "Where is X defined?" | `find_symbol(name_path="X")` | Grep |
| "Who calls X?" | `node_edges(id=..., kinds=["callers"])` | Grep |
| "What does X call?" | `node_edges(id=..., kinds=["callees"])` | Read whole file |
...
3. **`search`** (BM25 over name + doc) / **`find_symbol`** (glob over name-path) — locate the target.
...
7. **`find_code`** — AST-aware (tree-sitter) or regex across files.
